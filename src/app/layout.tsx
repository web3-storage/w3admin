      ...
      <div className='flex flex-col justify-between h-full'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <div className='flex flex-row space-x-2'>
            {navLinks.map(link => (
              <a key={`${link.href}-${link.name}`} className='text-xs block text-center mt-2' href={link.href}>{link.name}</a>
            ))}
          </div>
        </div>
      </div>
...
export default function RootLayout ({ sidebar = <div></div>, children }: LayoutComponentProps): JSX.Element {
  ...
  return (
    <AgentProvider>
      <ServiceProvider>
        <html>
          <body>
            <div className='flex min-h-full w-full bg-gray-800 text-white'>
